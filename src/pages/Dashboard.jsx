    ...
    <div style={{ ...s.container, background: theme.gradientBg }}>
      ...
      <div style={s.header}>
        <div style={s.headerLeft}>
          ...
          <span style={{
            ...
            color: theme.primary,
            border: `1px solid ${theme.primaryGlow}`,
        ...
        <div style={s.headerRight}>
          ...
          <div style={{ ...s.avatar, background: theme.gradient1 }}>
            {user?.name?.charAt(0)}
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={s.cardHeader}>
          <h3 style={{ ...s.sectionTitle, color: theme.text }}>
        ...
        {todayStatus && !todayStatus.checkedIn && (
          <button onClick={handleCheckIn} style={s.checkInBtn}>
            ...
            <div>
              <p style={s.checkBtnLabel}>{t('checkIn')}</p>
              <p style={s.checkBtnSub}>Tap to start your day</p>
            </div>
          ...
          <div>
            <div style={s.timeInfoRow}>
              <div style={{ ...s.timeInfoBox, background: theme.successLight }}>
                ...
                <p style={{ ...s.timeInfoValue, color: theme.text }}>
                  {new Date(todayStatus.attendance?.checkIn?.time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
            ...
            <button onClick={handleCheckOut} style={s.checkOutBtn}>
              <div style={s.checkBtnIcon}>🏠</div>
              <div>
                <p style={s.checkBtnLabel}>{t('checkOut')}</p>
                <p style={s.checkBtnSub}>End your work day</p>
              </div>
              <span style={s.checkBtnArrow}>→</span>
            </button>
          </div>
          ...
          <div>
            <div style={s.completedGrid}>
              <div style={{ ...s.completedItem, background: theme.successLight }}>
                ...
                <p style={{ color: theme.text, fontSize: '18px', fontWeight: '800' }}>
                  {new Date(todayStatus.attendance?.checkIn?.time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
              <div style={{ ...s.completedItem, background: theme.warningLight }}>
                ...
                <p style={{ color: theme.text, fontSize: '18px', fontWeight: '800' }}>
                  {new Date(todayStatus.attendance?.checkOut?.time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
              <div style={{ ...s.completedItem, background: theme.primaryLight }}>
                ...
                <p style={{ color: theme.text, fontSize: '18px', fontWeight: '800' }}>
                  {todayStatus.attendance?.workHours}h
                </p>
              </div>
            </div>
            <div style={{ ...s.doneBar, background: theme.successLight, borderColor: theme.success }}>
              <span>✅</span>
              <span style={{ color: theme.success, fontWeight: '700' }}>{t('workDone')}</span>
            </div>
          </div>
        )}
      </div>
      ...
      {isAdmin && (
        <div style={s.statsRow}>
          <div
            style={{ ...s.statCard, background: `linear-gradient(135deg, rgba(129,140,248,0.1) 0%, rgba(167,139,250,0.05) 100%)`, border: `1px solid ${theme.primaryGlow}` }}
          ...
          <div
            style={{ ...s.statCard, background: `linear-gradient(135deg, rgba(251,191,36,0.1) 0%, rgba(245,158,11,0.05) 100%)`, border: `1px solid rgba(251,191,36,0.2)` }}
      ...
      <div style={s.section}>
        <div style={s.sectionHeader}>
          ...
          <span style={{ ...s.seeAll, color: theme.primary }} onClick={() => navigate('/attendance')}>
            {t('viewAll')} →
          </span>
        </div>
        {myAttendances.map((att, index) => (
          ...
          >
            <div style={s.recordLeft}>
              <div style={{
                ...s.recordDot,
                background: att.status === 'present' ? theme.success : theme.warning
              }}></div>
              <div>
        ...
        {myAttendances.length === 0 && (
          <div style={{ ...s.emptyState, color: theme.textMuted }}>
      ...
      {isAdmin && (
        <div style={s.section}>
          <h3 style={{ ...s.sectionTitle, color: theme.text, marginBottom: '14px' }}>
            <span style={s.sectionDot}></span>
            {t('management')}
          </h3>
...
const s = {
  ...
  menuCard: { padding: '20px 12px', borderRadius: '18px', textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s' },
};
